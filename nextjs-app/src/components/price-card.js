...
export default function PriceCard({
  ...
  return (
    <Card
      className={header ? 'package__card active' : 'packages__card'}
      sx={styles.pricingBox}
    >
      {header && <Text sx={styles.header}>{header}</Text>}
      <Box>
        <Box className='package__header' sx={styles.pricingHeader}>
          <Heading className='pacckage__name' variant="title">{name}</Heading>
          <Text as="p">{description}</Text>
        </Box>
        <List items={points} childStyle={styles.listItem}/>
        <Text className="packages__pricing" sx={styles.price}>
          {priceWithUnit}
          <span>/Monthly</span>
        </Text>
